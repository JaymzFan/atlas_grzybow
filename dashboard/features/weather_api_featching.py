...
def from_utc_unix(
    unix_timestamp: float, hour_shift: int = 0, output_day: bool = False
) -> str:
...
class WeatherAPI:
    def __init__(
        self,
        API_KEY: str,
        base_url: str = "https://api.openweathermap.org/data/2.5/onecall",
    ):
    ...
    def __transform_response(api_response: requests.Response) -> List:
        ...
        next_days = [
            {
                "dzien": from_utc_unix(
                    unix_timestamp=x["sunrise"],
                    hour_shift=hours_offset,
                    output_day=True,
                ),
                "wschod_slonca": from_utc_unix(
                    unix_timestamp=x["sunrise"], hour_shift=hours_offset
                ),
                "zachod_slonca": from_utc_unix(
                    unix_timestamp=x["sunset"], hour_shift=hours_offset
                ),
    ...
    def fetch_weather(
        self, lat: float, lon: float, exclude: str = "minutely,alerts", lang: str = "pl"
    ) -> List:
