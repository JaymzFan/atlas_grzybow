...
def fetch_locations_data(owner_id):
    ...
    data = db.fetch_locations_data(owner_id=owner_id, shared_with_user_id=owner_id)
...
def register_callbacks(dash_app):
    @dash_app.callback(
        Output("main_page_lokalizacje_layer", "children"),
    ...
    @dash_app.callback(
        Output("weather_forecast_placeholder", "children"),
    ...
    def get_weather_forecast2(curr_data):
        ...
        click_lat_lng = [
            round(x, ndigits=1)
            for x in [curr_data["location"]["lat"], curr_data["location"]["lng"]]
        ]
        today_date = datetime.now().date()

        pogoda_teraz = fetch_weather(
            today=today_date, lat=click_lat_lng[0], lon=click_lat_lng[1]
        )

        return render_weather_tables(weather_forecast=pogoda_teraz)

    @dash_app.callback(
        Output("locations_filters", "is_open"),
    ...
    @dash_app.callback(
        [
            Output("locations_filters_loctypes", "value"),
    ...
    def wyczysc_filtry(
        n_clicks, n_clicks_seasonal_mushrooms, location_types, mushrooms_options
    ):
        if n_clicks == 0:
            raise PreventUpdate
        ...
        if trigger == "button-mark-sesonal-mushrooms":
            ...
            seasonal_mushrooms = [
                x[0] for x in seasonal_mushrooms_info.items() if x[1] is True
            ]
            all_mushrooms = [
                x["value"]
                for x in mushrooms_options
                if x["value"] in seasonal_mushrooms
            ]
    ...
    @dash_app.callback(
        Output("store-all-locations-data", "data"),
        Input("url", "pathname"),
    ...
    @dash_app.callback(
        Output("store-filtered-locations-ids", "data"),
    ...
    def filter_locations(
        n_clicks, n_clics_clear, loc_types, mushroom_types, all_locations
    ):
        ...
        filt_loc = [x for x in all_locations if x["prywatnosc"] in loc_types]
    ...
    @dash_app.callback(
        Output("locations_filters_mushroomtypes", "options"),
    ...
    @dash_app.callback(
        Output("locations_filtered_number", "children"),
    ...
    @dash_app.callback(
        Output("store-current-location-data", "data"),
    ...
    @dash_app.callback(
        [
            Output("clicked_loc_name", "children"),
    ...
    def load_current_location_card(location_data):
        ...
        mushrooms_list = [
            dbc.ListGroupItem(
                [html.Div([x, " ", seasonal_mushrooms[x]["badge"]])],
                color=seasonal_mushrooms[x]["color"],
            )
            for x in location_data["Grzyby"]
        ]

        return location_data["Nazwa"], location_data["Opis"], mushrooms_list

    @dash_app.callback(
        Output("locations_markers", "data"),
        [
            Input("locations_clear_filters", "n_clicks"),
            Input("store-filtered-locations-ids", "data"),
        ],
    ...
    @dash_app.callback(
        Output("locations_circles", "children"),
        [
            Input("locations_clear_filters", "n_clicks"),
            Input("store-filtered-locations-ids", "data"),
        ],
    ...
    def render_filtered_locations_circles(n_clicks, filtered_locations, all_locations):
        ...
        if trigger == "locations_clear_filters" and all_locations is not None:
            return [
                dl.Circle(
                    center=[x["location"]["lat"], x["location"]["lng"]],
    ...
    @dash_app.callback(
        [
            Output("loc_friends_shared_with", "options"),
    ...
    def show_friends_to_share_location(loc_data, un):
        ...
        options = [
            {"label": x["username"], "value": x["id"], "disabled": opt_disabled}
    ...
    def modify_location(loc_data, all_loc):
        ...
        return (
            loc_data["Nazwa"],
            loc_data["Opis"],
            loc_data["location"]["radius_in_meters"],
            all_mushrooms_opt,
            loc_data["Grzyby"],
        )

    @dash_app.callback(
        Output("modify-loc-tab", "disabled"),
        Input("store-current-location-data", "data"),
    )
    ...
    @dash_app.callback(
        [
            Output("button-modify-loc-submit", "disabled"),
    ...
    @dash_app.callback(
        Output("location_info_tabs", "active_tab"),
        Input("store-current-location-data", "data"),
    )
